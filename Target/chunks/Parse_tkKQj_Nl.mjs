  ...
  for (const Package of Option[0]) {
    ...
    try {
      if (Match(Package, /GitHub:/)) {
        ...
        if (JSONGitHub?.data) {
          Items.add({
            ...
            Name: typeof Package === "object" ? Object.values(Package).at(0)?.Name ?? JSONGitHub.data?.name : JSONGitHub.data?.name ?? "",
      ...
      if (Match(Package, /NPM:/)) {
        ...
        GitHub = (JSONNPM?.repository?.url ?? "")?.replace(/(git\+)?http?s:\/\/github.com\//, "")?.replace("#readme", "")?.replace(".git", "");
        Items.add({
          Link: `https://github.com/${GitHub}`,
          Name: typeof Package === "object" ? Object.values(Package).at(0)?.Name ?? GitHub.split("/")[1] : GitHub.split("/")[1] ?? "",
          GitHub,
      ...
      if (Match(Package, /cargo:/)) {
        const Crate = typeof Package === "string" ? Package.split(":")[1] : Object.keys(Package).entries().next().value.split(":")[1];
        const JSONCargo = await (await fetch(`https://crates.io/api/v1/crates/${Crate}`)).json();
        GitHub = (JSONCargo?.crate?.repository ?? "")?.replace(/(git\+)?http?s:\/\/github.com\//, "")?.replace("#readme", "")?.replace(".git", "");
        Items.add({
          Link: JSONCargo?.crate?.repository ?? "",
          Name: typeof Package === "object" ? Object.values(Package).at(0)?.Name ?? JSONCargo?.crate?.name : JSONCargo?.crate?.name ?? "",
          Badge: /* @__PURE__ */ new Set([
            {
...
const { default: Request } = await import('./Request_DFAZ0sve.mjs');

export { Match, Request, Parse as default };
//# sourceMappingURL=Parse_tkKQj_Nl.mjs.map
