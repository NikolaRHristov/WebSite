...
class C extends Event {
	...
	info;
	newDocument;
	constructor(t, n, o, r, c, f, d, s, m) {
		super(t, n),
			(this.from = o),
			(this.to = r),
			(this.direction = c),
			(this.navigationType = f),
...
class re extends C {
	...
	loader;
	constructor(t, n, o, r, c, f, d, s, m) {
		super(Z, { cancelable: !0 }, t, n, o, r, c, f, d),
			(this.formData = s),
...
async function ie(e, t, n, o, r, c, f, d) {
	const s = new re(e, t, n, o, r, c, window.document, f, d);
	return (
		document.dispatchEvent(s) &&
			(await s.loader(),
			s.defaultPrevented ||
				(oe(ee),
				s.navigationType !== "traverse" && k({ scrollX, scrollY }))),
		s
	);
}
...
const K = (e, t, n, o, r) => {
	...
	if (e.href !== location.href && !r)
		if (n.history === "replace") {
			...
			ce({ ...n.state, index: ++E, scrollX: 0, scrollY: 0 }, "", e.href);
	if (
		((document.title = f),
		(D = e),
		c || (scrollTo({ left: 0, top: 0, behavior: "instant" }), (d = !0)),
		r)
	)
		...
		if (e.hash) {
			...
			(location.href = e.href),
				history.state ||
					(S(s, ""),
					c && window.dispatchEvent(new PopStateEvent("popstate")));
	...
	for (const n of e.querySelectorAll("head link[rel=stylesheet]"))
		if (
			...
			o.setAttribute("rel", "preload"),
				...
				t.push(
					new Promise((r) => {
						["load", "error"].forEach((c) =>
							o.addEventListener(c, r),
						),
...
async function N(e, t, n, o) {
	const r = (i, l) => {
			const w = i.getAttribute(p),
				b = w && l.head.querySelector(`[${p}="${w}"]`);
			if (b) return b;
			...
			if (i?.closest(`[${p}]`)) {
				if (
					...
					const l = i.selectionStart,
						w = i.selectionEnd;
					return { activeElement: i, start: l, end: w };
				}
				return { activeElement: i };
			} else return { activeElement: null };
		},
		f = ({ activeElement: i, start: l, end: w }) => {
			i &&
				(i.focus(),
				(i instanceof HTMLInputElement ||
					i instanceof HTMLTextAreaElement) &&
					(typeof l == "number" && (i.selectionStart = l),
					typeof w == "number" && (i.selectionEnd = w)));
		},
		d = (i) => {
		...
		s = (i) => {
			const l = document.documentElement,
				w = [...l.attributes].filter(
					({ name: a }) => (
						l.removeAttribute(a), a.startsWith("data-astro-")
					),
				);
			[...i.newDocument.documentElement.attributes, ...w].forEach(
				({ name: a, value: u }) => l.setAttribute(a, u),
			);
			for (const a of document.scripts)
				for (const u of i.newDocument.scripts)
					if (
						!u.hasAttribute("data-astro-rerun") &&
						((!a.src && a.textContent === u.textContent) ||
							(a.src && a.type === u.type && a.src === u.src))
					) {
						u.dataset.astroExec = "";
						break;
					}
			for (const a of Array.from(document.head.children)) {
				const u = r(a, i.newDocument);
				u ? u.remove() : a.remove();
			}
			...
			for (const a of b.querySelectorAll(`[${p}]`)) {
				const u = a.getAttribute(p),
					v = document.querySelector(`[${p}="${u}"]`);
				v &&
					(v.replaceWith(a),
					v.localName === "astro-island" &&
						d(a) &&
						(a.setAttribute("ssr", ""),
						a.setAttribute("props", v.getAttribute("props"))));
			}
			f(T);
		};
	async function m(i) {
...
async function G(e, t, n, o, r) {
	...
	const c = r ? "traverse" : o.history === "replace" ? "replace" : "push";
	if (
		(c !== "traverse" && k({ scrollX, scrollY }),
		_(t, n) && ((e !== "back" && n.hash) || (e === "back" && t.hash)))
	) {
		K(n, t, o, document.title, r);
		return;
	}
	const f = await ie(t, n, e, c, o.sourceElement, o.info, o.formData, d);
	...
	async function d(s) {
		...
		if (s.formData) {
			...
			y.body =
				l?.attributes.getNamedItem("enctype")?.value ===
				"application/x-www-form-urlencoded"
					? new URLSearchParams(s.formData)
					: s.formData;
		}
		const h = await ue(m, y);
		if (h === null) {
			s.preventDefault();
			return;
		}
		if (
			(h.redirected && (s.to = new URL(h.redirected)),
			(M ??= new DOMParser()),
			(s.newDocument = M.parseFromString(h.html, h.mediaType)),
			s.newDocument
	...
	if (((L = !1), I))
		g = document.startViewTransition(async () => await N(f, o, r));
	else {
		const s = (async () => {
			await new Promise((m) => setTimeout(m)), await N(f, o, r, j());
		})();
		g = {
...
function ye() {
	...
	H(() => {
		for (const t of document.getElementsByTagName("a"))
			P.has(t) ||
				(A(t, "viewport") && (P.add(t), (e ??= ge()), e.observe(t)));
	});
}
function ge() {
	const e = new WeakMap();
...
(I || ve() !== "none") &&
	...
	document.addEventListener("submit", (e) => {
		...
		let d = o?.getAttribute("formaction") ?? c ?? location.pathname;
		const s = o?.getAttribute("formmethod") ?? f ?? "get";
		if (
			s === "dialog" ||
			location.origin !== new URL(d, location.href).origin
		)
			return;
		const m = { sourceElement: o ?? n };
		if (s === "get") {
			const y = new URLSearchParams(r),
				h = new URL(d);
			(h.search = y.toString()), (d = h.toString());
		} else m.formData = r;
		e.preventDefault(), F(d, m);
	}),
	he({ prefetchAll: !0 }));
//# sourceMappingURL=ViewTransitions.astro_astro_type_script_index_0_lang.BMqMAa2A.js.map
