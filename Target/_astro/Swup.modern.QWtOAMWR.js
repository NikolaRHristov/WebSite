const M = new WeakMap();
function D(t, e, i, n) {
	if (!t && !M.has(e)) return !1;
	const s = M.get(e) ?? new WeakMap();
	M.set(e, s);
	...
	const { once: r, ...a } = n,
		...
		h = (n) => {
			...
			if (s) {
				...
				i.call(c, t),
					r && (c.removeEventListener(e, h, a), D(!1, c, i, u));
			}
		},
		u = JSON.stringify({ selector: t, type: e, capture: l });
	D(!0, c, i, u) || c.addEventListener(e, h, a),
...
function rt(t, e, i) {
	...
	if (c)
		...
		var S = t[t.length - 1],
			x =
				"string" == typeof S
					? m.indexOf(S[S.length - 1]) > -1
					: void 0 === S;
		s || (v += "(?:".concat(m, "(?=").concat(f, "))?")),
			x || (v += "(?=".concat(m, "|").concat(f, ")"));
	}
	return new RegExp(v, B(i));
}
function F(t, e, i) {
...
const K = (t, e) =>
	...
	at = (t, e, i, n) => {
		...
		return (
			Q(t, e, i, (n = y({}, n, { signal: s.signal }))),
			{ destroy: () => s.abort() }
		);
	};
let A = class t extends URL {
...
async function ct(t, e = {}) {
	...
	try {
		...
		throw l
			? (this.hooks.call("fetch:timeout", n, { url: t }),
				new $(`Request timed out: ${t}`, { url: t, timedOut: l }))
			: "AbortError" === e?.name || a.aborted
				? new $(`Request aborted: ${t}`, { url: t, aborted: !0 })
				: e;
	}
	...
	if (!p) throw new $(`Empty response: ${d}`, { status: u, url: d });
	const { url: f } = A.fromUrl(d),
		m = { url: f, html: p };
	return (
		!n.cache.write ||
...
const q = (t, e = document) => e.querySelector(t),
	V = (t, e = document) => Array.from(e.querySelectorAll(t)),
	z = () =>
		new Promise((t) => {
...
const W = (t) => (window.CSS && window.CSS.escape ? CSS.escape(t) : t),
	_ = (t) => 1e3 * Number(t.slice(0, -1).replace(",", "."));
class ut {
	constructor(t) {
		(this.swup = void 0),
			(this.swupClasses = [
...
class ft {
	...
	async call(t, e, i, n) {
		...
		const [h] = await this.run(c, s, o, !0);
	...
	async run(t, e = this.swup.visit, i, n = !1) {
		const s = [];
		for (const { hook: o, handler: r, defaultHandler: a, once: c } of t)
			if (null == e || !e.done) {
				c && this.off(o, r);
				try {
					const t = await ht(r, [e, i, a]);
					s.push(t);
				} catch (t) {
					if (n) throw t;
	...
	runSync(t, e = this.swup.visit, i, n = !1) {
		const s = [];
		for (const { hook: o, handler: r, defaultHandler: a, once: c } of t)
			if (null == e || !e.done) {
				c && this.off(o, r);
...
const pt = (t) => {
	...
	U = "transition",
	H = "animation";
async function mt({ elements: t, selector: e }) {
	if (!1 === e && !t) return;
	let i = [];
	if (t) i = Array.from(t);
	else if (e && ((i = V(e, document.body)), !i.length))
			...
			return (
				...
				new Promise((s) => {
					...
					const c = () => {
						...
						l = (e) => {
							if (e.target === t) {
								if (![`${U}end`, `${H}end`].includes(e.type))
									throw new Error(
...
function j(t, e) {
	...
	return Math.max(...e.map((e, i) => _(e) + _(t[i])));
}
function vt(t, e = {}, i = {}) {
	if ("string" != typeof t)
	...
	const { url: n, hash: s } = A.fromUrl(t),
		...
		if (
			...
			"replace" === t.history.action || t.to.url === this.currentPageUrl
				...
				: (this.currentHistoryIndex++,
					ot(e, { index: this.currentHistoryIndex }));
		}
		if (
			((this.currentPageUrl = k()),
			t.history.popstate && this.classes.add("is-popstate"),
			t.animation.name && this.classes.add(`to-${K(t.animation.name)}`),
...
const wt = async function (t) {
	...
	yt = function (t) {
		...
		const n =
			...
			"";
		document.title = n;
		const s = V('[data-swup-persist]:not([data-swup-persist=""])'),
			o = t.containers
				.map((t) => {
					const e = document.querySelector(t),
						n = i.querySelector(t);
					return e && n
						? (e.replaceWith(n.cloneNode(!0)), !0)
						: (e ||
								console.warn(
			...
			await this.hooks.call(
				...
				(t, {}) => {
					...
					t.animation.animate &&
						...
						t.animation.name &&
							this.classes.add(`to-${K(t.animation.name)}`));
				}
			),
...
class $t {
	constructor(t = {}) {
		...
		(this.version = "4.6.0"),
			...
			(this.currentHistoryIndex =
				null !=
				(e = null == (i = window.history.state) ? void 0 : i.index)
					? e
					: 1),
	...
	async enable() {
		...
		(this.clickDelegate = this.delegateEvent(
			...
			"swup" !==
				(null == (t = window.history.state) ? void 0 : t.source) &&
	...
	shouldIgnoreVisit(t, { el: e, event: i } = {}) {
		const { origin: n, url: s, hash: o } = A.fromUrl(t);
		return (
	...
	handleLinkClick(t) {
		const e = t.delegateTarget,
			{ href: i, url: n, hash: s } = A.fromElement(e);
	...
	handlePopState(t) {
		...
		if (
			...
			return;
		const { url: r, hash: a } = A.fromUrl(o),
