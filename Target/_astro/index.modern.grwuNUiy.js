import { e as s } from "./index.modern.kBZUKyzZ.js";
class i extends s {
	constructor(...t) {
		super(...t),
			...
			(this._addClassNameToElement = () => {
				this._classNameAddedToElements.forEach((t) => {
					Array.from(document.querySelectorAll(t.selector)).forEach(
						(e) => {
							e.classList.add(`swup-transition-${t.name}`);
						}
					);
				});
			});
				...
				Array.from(document.querySelectorAll(t.selector)).forEach(
					(t) => {
						t.className.split(" ").forEach((e) => {
							new RegExp("^swup-transition-").test(e) &&
...
class l extends i {
	...
	createOverlay() {
		...
		return `\n\t\t\t<div\n\t\t\t\tclass="swup-transition-overlay"\n\t\t\t\tdata-direction="${t}"\n\t\t\t\tstyle="--swup-overlay-theme-color:${
			void 0 !== this.options.color ? this.options.color : "inherit"
		};--swup-overlay-theme-duration:${
			void 0 !== this.options.duration
